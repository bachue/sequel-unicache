describe Sequel::Unicache::Finder do
  ...
  context '.[]' do
    context 'simple pk' do
      ...
      it 'should get model from cache' do
        ...
        cache = {values: { id: 10, username: 'bachue@emc.com', password: '123456', company_name: 'EMC', department: 'DPC', employee_id: 1000 }, version: 1}
        memcache.set 'User:id:10', cache.to_yaml
        user = User[10]
        expect(user).not_to be_nil
        expect(user.values).to eq cache[:values]
      end
    end
    ...
    context 'single unicache key' do
      ...
      it 'should get model from cache' do
        ...
        cache = {values: { id: 10, username: 'bachue@emc.com', password: '123456', company_name: 'EMC', department: 'DPC', employee_id: 1000 }, version: 1}
        memcache.set 'User:username:bachue@emc.com', Marshal.dump(cache)
    ...
    context 'complexed unicache key' do
      ...
      it 'should get model from cache' do
        ...
        cache = {values: { id: 10, username: 'bachue@emc.com', password: '123456', company_name: 'EMC', department: 'DPC:Mozy', employee_id: 1000 }, version: 1}
        memcache.set 'User:company_name:EMC:department:DPC\:Mozy:employee_id:1000', Marshal.dump(cache)
  ...
  context '.find' do
    ...
    context 'single unicache key' do
      ...
      it 'should get model from cache' do
        ...
        cache = {values: { id: 10, username: 'bachue@emc.com', password: '123456', company_name: 'EMC', department: 'DPC', employee_id: 1000 }, version: 1}
        memcache.set 'User:username:bachue@emc.com', cache.to_yaml
  ...
  context 'association' do
    ...
    it 'should cache associated object' do
      ...
      expect(cache).not_to be_nil
      expect(Marshal.load(cache)).to eq values: manager.values, version: 1
    end

    it 'should get model from cache' do
      expect(user.manager.manager).to be_nil
      cache = {values: { id: 10, username: 'tim@emc.com', password: 'abcdef', company_name: 'EMC', department: 'DPC:Mozy', employee_id: 100 }, version: 1}
      memcache.set "User:id:10", Marshal.dump(cache)
      user.manager.set manager_id: 10
      manager = user.manager.manager
      expect(manager).not_to be_nil
      expect(manager.values).to eq cache[:values]
    end
  end
end
